...
class ModifierDataTable(View):
    def get(self, request):
        ...
        base_queryset = Modifier.objects.select_related('menu_item')
        ...
        if search_value:
            base_queryset = base_queryset.filter(
                ...
                Q(extra_price__icontains=search_value) |
        ...
        columns = ['id', 'name', 'menu_item', 'description', 'is_active', 'extra_price']
        ...
        for item in paginated_queryset:
            data.append({
                "id": item.id,
                "name": item.name,
                "menu_item": item.menu_item.name if item.menu_item else "-",
